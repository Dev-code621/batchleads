...
export default ({
  item, folders, newItem, hideDelete, onCreateFolder, showCount,
}) => {
  const updateResult = useSelector(folderUpdateSelector)
  const dispatch = useDispatch()
  const [value, setValue] = useState(item.name)
  const inputElement = useRef(null)
  const [editing, setEditing] = useState(newItem || false)
  ...
  return (
    <div className="property-status-item folder-edit-item">
      ...
      <input
        ...
        className={`folder-edit-input${editing ? ' selected' : ''}`}
      />
      {!editing && showCount && (
      ...
      {editing ? (
        ...
        <div className="icon-container" onClick={onEdit}>
