...
import { tagUpdateSelector } from '~redux/selectors/tagSelector'
import { updateTag, deleteTag } from '~redux/modules/tag'
...
export default ({
  ...
  const updateResult = useSelector(tagUpdateSelector)
  ...
  const onSave = () => {
    ...
    if (newItem) {
      onCreateTag(item.id, value)
      return
    }
    const originTag = tags.find(((fItem) => fItem.id === item.id))
    if (originTag && originTag.name !== value) {
      dispatch(updateTag(item.id, value))
    }
  }
  const keyPressed = (e) => {
    if (e.key === 'Enter') {
  ...
  return (
    <div className="tag-row-item">
      <div className="tag-row-item-container">
        <div className="property-status-item tag-edit-item">
          <input
            type="text"
            ref={inputElement}
            value={value}
            ...
            className={`tag-edit-input${setEdit ? ' selected' : ''}`}
          />
          {setEdit ? (
            <Fragment>
